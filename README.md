...
- OSV Scanner runs for pull requests, pushes to `main`, and manual CI runs.
- OSV scans the Maven CycloneDX aggregate SBOM generated by
  `./mvnw -B -ntp -Psbom -DskipTests verify`; it does not recursively scan the repository in CI.
- CodeQL remains the code security analysis gate.
...
- Release is not performed by `ci.yml`; `.github/workflows/release.yaml` starts from a release tag
  push, waits for `Checks / Main` to pass on the same commit, then deploys docs and creates the
  GitHub Release.
